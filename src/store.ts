...
export async function reindexCollection(
  ...
  // Skip collections whose directory no longer exists on disk
      ...
      for (const batchMeta of batches) {
        ...
        for (
          ...
          if (!session.isValid) {
            ...
            errors += remaining;
            console.warn(
              `⚠ Session expired — skipping ${remaining} remaining chunks`,
            );
            break;
          }
          ...
          if (processed >= BATCH_SIZE && errors > processed * 0.8) {
            ...
            errors += remaining;
            console.warn(
              `⚠ Error rate too high (${errors}/${processed}) — aborting embedding`,
            );
            break;
          }
...
export function createStore(dbPath?: string): Store {
  ...
  const store: Store = {
    ...
    expandQuery: (query: string, model?: string, intent?: string) =>
      expandQuery(query, model, db, intent, store.llm),
    rerank: (
      query: string,
      documents: { file: string; text: string }[],
      model?: string,
      intent?: string,
    ) => rerank(query, documents, model, db, intent, store.llm),
...
export async function hybridQuery(
  ...
  if (hasVectors) {
    ...
    const textsToEmbed = vecQueries.map((q) =>
      formatQueryForEmbedding(q.text, llm.embedModelName),
    );
  ...
  if (hasVectors) {
    ...
    if (vecSearches.length > 0) {
      ...
      const textsToEmbed = vecSearches.map((s) =>
        formatQueryForEmbedding(s.query, llm.embedModelName),
      );
      hooks?.onEmbedStart?.(textsToEmbed.length);
      const embedStart = Date.now();
      const embeddings = await llm.embedBatch(textsToEmbed);
