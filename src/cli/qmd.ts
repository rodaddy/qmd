...
import {
  getEmbeddedQmdSkillContent,
  getEmbeddedQmdSkillFiles,
...
async function showStatus(): Promise<void> {
  ...
  try {
    ...
    if (device.gpu) {
      console.log(
        `  GPU:      ${c.green}${device.gpu}${c.reset} (offloading: ${device.gpuOffloading ? "yes" : "no"})`,
      );
      ...
      console.log(
        `  GPU:      ${c.yellow}none${c.reset} (running on CPU — models will be slow)`,
      );
      console.log(
        `  ${c.dim}Tip: Install CUDA, Vulkan, or Metal support for GPU acceleration.${c.reset}`,
      );
    }
    console.log(`  CPU:      ${device.cpuCores} math cores`);
  } catch (error) {
    console.log(
      `  Status:   ${c.dim}skipped${c.reset} (status probe does not build llama.cpp backends)`,
    );
...
async function vectorIndex(
  ...
  const result = await generateEmbeddings(storeInstance, {
    ...
    onProgress: (info) => {
      ...
      const errStr =
        info.errors > 0 ? ` ${c.yellow}${info.errors} err${c.reset}` : "";

      if (isTTY)
        process.stderr.write(
          `\r${c.cyan}${bar}${c.reset} ${c.bold}${percentStr}%${c.reset} ${c.dim}${info.chunksEmbedded}/${info.totalChunks}${c.reset}${errStr} ${c.dim}${throughput} ETA ${eta}${c.reset}   `,
        );
    },
  });
...
function outputResults(
  ...
  if (opts.format === "json") {
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      const virtualPath = row.file.startsWith("qmd://")
        ? row.file
        : toQmdPath(row.displayPath);
      ...
      if (process.stdout.isTTY && absolutePath && parsed?.path) {
        ...
        console.log(
          `${c.cyan}${legacyPath}${c.dim}${lineInfo}${c.reset}${docidStr}`,
        );
      }
