...
| 6 | `qmd embed` exits 1 after the error-rate abort | `src/store.ts` (`EmbedResult.aborted`, `generateEmbeddings`), `src/cli/qmd.ts` (embed command) | Upstream aborts once >80% of attempted chunks fail, writes the rest off, warns, and still exits 0, so a scheduler recording exit codes logs a truncated pass as success (qmd#5). | `qmd embed` exits 0 after printing "Error rate too high", or `EmbedResult` has no `aborted` field. |
| 7 | Remote embed repairs lone surrogates before the wire | `src/remote-llm.ts` (`post`, `wellFormedStrings`) | A chunk boundary can split a surrogate pair; `JSON.stringify` emits the half as `\udf4c` and llama-server answers 500 (`surrogate U+DC00..U+DFFF must follow U+D800..U+DBFF`), failing the whole batch for one character. Seen on skippy-agentspace, 2026-08-28. | A `qmd embed` batch fails with that 500 text, or `src/remote-llm.ts` has no `wellFormedStrings` replacer. |
